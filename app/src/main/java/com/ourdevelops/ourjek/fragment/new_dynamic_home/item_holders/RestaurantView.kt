...
import com.ourdevelops.ourjek.databinding.RestaurantViewBinding
...
class RestaurantView(itemView: View) : RecyclerView.ViewHolder(itemView) {
    private val binding = RestaurantViewBinding.bind(itemView)
    private val mContext = itemView.context

    fun setUpView(data: VerticalListModel?) {
        binding.nama.text = data?.nama_merchant
        if (!data?.foto_merchant.isNullOrEmpty()) {
            Glide.with(mContext)
        ...
        binding.distance.text = data.alamat_merchant
        binding.restDesc.text = "$format kms"
        ...
        binding.llHomeRestaurant.setOnClickListener {
            ...
            intent.putExtra(IntentKeys.filter, data.id_fitur)
